...
<policies>
  <inbound>
      ...
      <validate-jwt header-name="Authorization" failed-validation-httpcode="401" failed-validation-error-message="JWT validation was unsuccessful" require-expiration-time="true" require-scheme="Bearer" require-signed-tokens="true">
          ...
          <audiences>
          <audience>${local.event_ingest_api.application.primary_identifier_uri}</audience>
          </audiences>
          <issuers>
      ...
      <authentication-managed-identity resource="${local.event_ingest_api.application.primary_identifier_uri}" output-token-variable-name="msi-access-token" ${format(" client-id=\"%s\"", local.api_management_identity.client_id)} />
